...
// GitConfig Git操作相关配置
// 配置Git仓库扫描的行为和限制，支持本地仓库和GitLab API
type GitConfig struct {
	...
	// GitLab配置
	GitLab GitLabConfig `mapstructure:"gitlab" yaml:"gitlab"` // GitLab API配置
}

// GitLabConfig GitLab API配置
// 配置GitLab服务器连接和认证信息
type GitLabConfig struct {
	Enabled    bool   `mapstructure:"enabled" yaml:"enabled"`         // 是否启用GitLab集成
	BaseURL    string `mapstructure:"base_url" yaml:"base_url"`       // GitLab服务器地址，如：https://gitlab.com 或 https://git.infoloop.cn
	Username   string `mapstructure:"username" yaml:"username"`       // GitLab用户名
	Password   string `mapstructure:"password" yaml:"password"`       // GitLab密码或访问令牌
	Token      string `mapstructure:"token" yaml:"token"`             // GitLab Personal Access Token（推荐）
	ProjectIDs []int  `mapstructure:"project_ids" yaml:"project_ids"` // 要监控的项目ID列表
}
...
type FeishuConfig struct {
	...
	WebhookSecret string `mapstructure:"webhook_secret" yaml:"webhook_secret"` // 飞书机器人Webhook密钥
...
type ReportConfig struct {
	DefaultTemplate string            `mapstructure:"default_template" yaml:"default_template"` // 默认模板
	Templates       map[string]string `mapstructure:"templates" yaml:"templates"`               // 自定义模板路径
	AI              AIConfig          `mapstructure:"ai" yaml:"ai"`                             // AI配置
...
type AIConfig struct {
	Enabled  bool   `mapstructure:"enabled" yaml:"enabled"`   // 是否启用AI生成
	Provider string `mapstructure:"provider" yaml:"provider"` // AI提供商: ollama, openai, gemini, qwen

	// Ollama配置
	OllamaURL   string `mapstructure:"ollama_url" yaml:"ollama_url"`     // Ollama服务地址
	OllamaModel string `mapstructure:"ollama_model" yaml:"ollama_model"` // 使用的模型

	// OpenAI配置
	OpenAIKey   string `mapstructure:"openai_key" yaml:"openai_key"`     // OpenAI API Key
	OpenAIModel string `mapstructure:"openai_model" yaml:"openai_model"` // OpenAI模型
	OpenAIURL   string `mapstructure:"openai_url" yaml:"openai_url"`     // OpenAI API URL (可选，支持代理)

	// Gemini配置
	GeminiKey   string `mapstructure:"gemini_key" yaml:"gemini_key"`     // Gemini API Key
	GeminiModel string `mapstructure:"gemini_model" yaml:"gemini_model"` // Gemini模型

	// 通义千问配置
	QwenKey   string `mapstructure:"qwen_key" yaml:"qwen_key"`     // 通义千问API Key
	QwenModel string `mapstructure:"qwen_model" yaml:"qwen_model"` // 通义千问模型

	// 通用配置
	Temperature float64 `mapstructure:"temperature" yaml:"temperature"` // 生成温度
	MaxTokens   int     `mapstructure:"max_tokens" yaml:"max_tokens"`   // 最大token数
	Timeout     int     `mapstructure:"timeout" yaml:"timeout"`         // 请求超时时间(秒)
}
